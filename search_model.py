...
class NASCell(nn.Module):
    def __init__(self, steps, device, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev, loose_end=False,
                 concat=None, op_type='FULLY_CONCAT_PRIMITIVES'):
...
class NASNetwork(nn.Module):
    def __init__(self, C, num_classes, layers, criterion, device, steps=4, stem_multiplier=3, controller_hid=None,
                 entropy_coeff=[0.0, 0.0], edge_hid=100, transformer_nfeat=1024, transformer_nhid=512,
                 transformer_dropout=0, transformer_normalize=False, loose_end=False, normal_concat=None,
                 reduce_concat=None, op_type='FULLY_CONCAT_PRIMITIVES'):
        ...
        for i in range(layers):
            ...
            cell = NASCell(steps, device, multiplier, C_prev_prev, C_prev, C_curr, reduction, reduction_prev,
    ...
    def _initialize_arch_transformer(self):
        self.arch_transformer = ArchTransformer(self._steps, self._device, self.edge_hid, self.transformer_nfeat,
                                                self.transformer_nhid, self.transformer_dropout,
                                                self.transformer_normalize, op_type=self.op_type)
    ...
    def _loss_transformer(self, input, target, baseline=None):
        logits, optimized_logits, optimized_normal, optimized_normal_logP, optimized_normal_entropy, optimized_reduce, \
                                    optimized_reduce_logP, optimized_reduce_entropy = self.transformer_forward(input)

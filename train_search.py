...
parser = argparse.ArgumentParser("ImprovedNAT")
...
if "SEVEN_JOB_ID" in os.environ:
    args.save = '{}-NATv2-search-{}'.format(os.environ['SEVEN_JOB_ID'], time.strftime("%Y%m%d-%H%M%S"))
else:
    args.save = 'NATv2-search-{}-{}'.format(args.save, time.strftime("%Y%m%d-%H%M%S"))
...
def main():
    ...
    model = Network(
        args.init_channels, CIFAR_CLASSES, args.layers, criterion, device, steps=args.num_steps,
        controller_hid=args.controller_hid, entropy_coeff=args.entropy_coeff, edge_hid = args.edge_hid,
        transformer_nfeat = args.transformer_nfeat, transformer_nhid = args.transformer_nhid,
        transformer_dropout = args.transformer_dropout, transformer_normalize = args.transformer_normalize,
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch+1) % args.test_freq == 0:
            ...
            # save model
        if args.store == 1 and (epoch+1) % 20 == 0:
            utils.save(model, os.path.join(args.save, 'models_epoch%d.pt' % epoch))
